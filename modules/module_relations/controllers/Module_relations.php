...
die(); */
        foreach($rows as $row) {
            ...
            $row_data['id_assoc'] = $row->id;
            $row_data['para_module'] = $alt_module;
            $value = '';
            foreach($bits as $bit) {
                $bit = trim($bit);
                $value.= $row->$bit.' ';
...
/*         foreach ($available_records as $key => $value){
    ...
    function _fetch_options($selected_key, $calling_module_name, $alt_module_name) {
        ...
        if ($relationship_type == 'one to many') {
            ...
            $sql = 'select * from '.$alt_module_name.' order by '.$order_by.' desc';
            $rows = $this->model->query($sql, 'object');
            ...
            foreach($rows as $row) {
                ...
                if ($selected_key == $row->id) {
                    $row_label = $value;
                    $options[0] = strtoupper('*** Desasociar con '.$row_label.' ***');
                } else {
    ...
    function _get_parent_options($parent_module_name, $identifier_column, $selected_key) {
        ...
        $options = [];
        $sql = 'select id, '.$identifier_column.' from '.$parent_module_name.' order by '.$identifier_column.' desc';
        $rows = $this->model->query($sql, 'object');
