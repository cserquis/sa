...
class My_filezone extends Trongate {
    ...
    function _draw_summary_panel($update_id, $filezone_settings) {
        ...
        $data['target_directory'] = BASE_URL.'pictures/'.$data['target_module'].'_pictures_thumb/'.$update_id.'/';
    ...
    function _make_sure_got_sub_folder($update_id, $filezone_settings) {
        ...
        $destination_media = $filezone_settings['destination_media'];
    ...
    function _get_pictures_directory($filezone_settings) {
        ...
        $directory = 'pictures/'.$target_module . '_pictures';
        return $directory;
    }

    function _get_pictures_directory_media($uploader_settings) {

        $target_module = $uploader_settings['targetModule'];
        $directory = 'pictures/'.$target_module . '_pictures_media';
        return $directory;
    }
    ...
    function _get_pictures_directory_thumb($uploader_settings) {
        ...
        $target_module = $uploader_settings['targetModule'];
        $directory = 'pictures/'.$target_module . '_pictures_thumb';
        return $directory;
    }
    ...
    function uploader() {
        ...
        $this->_make_sure_got_sub_folder($update_id, $settings);
    ...
    function _delete_picture_from_pictures($picture_name, $destination_thumb,$target_module, $update_id) {
        ...
        $picture_obj = $this->model->get_one_where('picture_name', $picture_name, 'pictures');
        ...
        $target_file_media = str_replace('projects_pictures_thumb', 'projects_pictures_media', $target_file_thumb);
    ...
    function _remove_picture($target_module, $update_id) {
        ...
        if($picture_obj == true) {
            ...
            $picture_path_ori = './pictures/' . $target_module . '_pictures/' . $update_id . '/' .$picture_name;
            if (file_exists($picture_path_ori)) {
                unlink($picture_path_ori);
            } 
            $picture_path_media = str_replace('_pictures/', '_pictures_media/', $picture_path_ori);
            if (file_exists($picture_path_media)) {
                unlink($picture_path_media);
            } 
    ...
    function _do_upload($update_id, $target_module) {
        ...
        $config['max_width']  = $filezone_settings['max_width'];
        $config['max_height'] = $filezone_settings['max_height'];
        ...
        $config['destination_media']  = $filezone_settings['destination_media'] . '/' . $update_id;

        $config['max_width_media'] = $filezone_settings['max_width_media'];
        ...
        $destination_media = $config['destination_media'];
        if(strlen($destination_media)>0) {
            $target_dir = APPPATH.'public/'.$destination_media;
            if (!file_exists($target_dir)) {
    ...
    private function save_the_pic($data) {
        ...
        if(isset($data['targetModule'])) {
            $ditch = '../public/pictures/'.$data['targetModule'].'_pictures/'.$data['update_id'].'/';

            $picture_name = str_replace($ditch, '',$filename );

            $picture_name = $data['picname'];
     
            $datap['picture_name'] = $picture_name;
            $datap['priority'] = 1;
        ...
        if (!is_dir($config['destination_media'])) {
            //Directory does not exist, so lets create it.
            mkdir($config['destination_media'], 0755);
        }

        
        $filename2 = str_replace('_pictures', '_pictures_media', $filename);
        $image->resizeToWidth(1500);
        $image->save($filename2, $compression);

            
            $filename3 = str_replace('_pictures', '_pictures_thumb', $filename);
            $image->resizeToWidth(900);
            $image->save($filename3, $compression);
